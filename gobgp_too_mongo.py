...
import bgp_attributes as BGP  # Assuming bgp_attributes.py defines BGP attribute constants
...
def get_update_entry(line):
    ...
    try:
        ...
        logging.error(f"Error in get_update_entry(line): {err}")
        return None
...
def build_json(update_entry):
    """Given an update entry from GoBGP, set the BGP attribute types as a
    ...
    for attribute in update_entry['attrs']:
        ...
        elif attribute['type'] == BGP.AS_PATH:
            try:
        ...
        elif attribute['type'] == BGP.ORIGINATOR_ID:
            update_json['originator_id'] = attribute['value']
        elif attribute['type'] == BGP.CLUSTER_LIST:
            update_json['cluster_list'] = attribute['value']
        elif attribute['type'] == BGP.MP_REACH_NLRI:
            update_json['nexthop'] = attribute['nexthop']
        elif attribute['type'] == BGP.MP_UNREACH_NLRI:
            logging.debug(f'Found MP_UNREACH_NLRI: {attribute}')
        elif attribute['type'] == BGP.EXTENDED_COMMUNITIES:
            logging.debug(f'Found EXTENDED_COMMUNITIES: {attribute}')
        elif attribute['type'] == BGP.LARGE_COMMUNITIES:
            try:
        ...
        try:
            ...
            if update_entry:
                ...
                if prefix_from_gobgp:
                    ...
                    logging.warning("build_json returned None for line:", line.strip())
            else:
                logging.warning("get_update_entry returned None for line:", line.strip())

        except TypeError as e:
            logging.error(f"TypeError {e} in Line: {line}")

        except Exception as e:
            logging.error(f"Exception {e} in Line: {line}")
